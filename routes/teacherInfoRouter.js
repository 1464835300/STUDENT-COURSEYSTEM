...
router.post("/delete", async (req, resp) => {
  let result = await ServiceFactory.createTeacherInfoService().deleteById(
    req.body
  );
...
router.post("/register", async (req, resp) => {
  let result = await ServiceFactory.createTeacherInfoService().register(
    req.body
  );
...
// 获取所有老师信息
router.post("/teacherList", async (req, resp) => {
  let result = await ServiceFactory.createTeacherInfoService().getAllList();
  let resultJson = new ResultJson(true, "教师信息获取成功", result);
  resp.json(resultJson);
});

// 验证登录名是否能用
router.post("/checkLoginname", async (req, resp) => {
  let result = await ServiceFactory.createTeacherInfoService().checkLoginname(
    req.body
  );
  let resultJson = new ResultJson(
    result,
    result ? "验证成功可以使用" : "登录名已被使用",
    result
  );
  resp.json(resultJson);
});
